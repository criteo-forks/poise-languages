...
module PoiseLanguages
  module Static
    ...
    module Mixin
      ...
      def static_checksum
        options['checksum'] % { checksum: new_resource.checksum }
      end
      ...
      module ClassMethods
        ...
        def default_inversion_options(node, resource)
          super.merge({
            ...
            url: static_url,
            # Checksum of file being downloaded
            checksum: static_checksum(node, resource),
          })
        end

        def static_options(name: nil, versions: [], machines: %w{linux-i686 linux-x86_64}, url: nil, checksum: nil, strip_components: 1, retries: 5)
        ...
        def static_checksum(node, resource)
          resource.checksum
        end
